 ...
 *
 * The main compliance dashboard — previously at pages/index.tsx.
 * Moved here so pages/index.tsx can serve the public landing page.
 * Auth-protected via _app.tsx.
 */
...
export default function Dashboard() {
  ...
  const statusBadge = (status: string) => {
    const map: Record<string, string> = {
      ...
      confirmed: 'badge-red', false_positive: 'badge-gray',
      closed: 'badge-green', ingested: 'badge-cyan',
    };
    return map[status] ?? 'badge-gray';
  };

  // Synthetic 24h chart data (replace with real data from API when available)
    ...
    <Layout>
      ...
      <div className="stat-grid mb-6">
        <div className="stat-card cyan">
          ...
          <div className="stat-change">All channels · NIBSS/POS/Mobile/M-PESA</div>
        </div>
        <div className="stat-card amber">
          <div className="stat-label">Open Alerts</div>
          <div className="stat-value">{stats?.open_alerts ?? alerts.length}</div>
          <div className="stat-change down">
            {(stats?.open_alerts ?? alerts.length) > 0 ? '⚑ Requires attention' : '—'}
          </div>
        </div>
        <div className="stat-card red">
          <div className="stat-label">High Risk (≥80%)</div>
          <div className="stat-value">{alerts.filter((a) => a.risk_score >= 0.8).length}</div>
          <div className="stat-change down">
            {alerts.filter((a) => a.risk_score >= 0.8).length > 0
                  ...
                  transactions.map((tx) => {
                    ...
                    return (
                      <tr key={tx.transaction_id}>
                        ...
                        <td style={{ color: 'var(--cyan)' }}>₦{tx.amount.toLocaleString()}</td>
                        <td>
